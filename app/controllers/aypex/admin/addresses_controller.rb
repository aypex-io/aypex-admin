module Aypex
  module Admin
    class AddressesController < ResourceController
      ...
      def new
        @address = Aypex::Address.new(country: current_store.default_country, user_id: params[:address][:user_id])
      end

      def update_country
        @address = Aypex::Address.new(update_country_params)
      ...
      def find_resources
        if params[:order_id]
          ...
          @user ||= Aypex::User.find(params[:address][:user_id]) if params[:address][:user_id].present?
          @user ||= @address.user
        end
      end
      ...
      def location_after_save
        if params[:address][:user_id].present?
          aypex.addresses_admin_user_path(@address.user)
        elsif params[:order_id].present?
          aypex.edit_admin_order_url(@order)
        end
      end
    end
  end
