    ...
    class TaxonsController < ResourceController
      ...
      def update
        ...
        if successful
          ...
          respond_with(@taxon) do |format|
            format.html { redirect_to aypex.edit_admin_taxonomy_taxon_path(@taxonomy.id, @taxon.id) }
          end
        else
          respond_with(@taxon) do |format|
      ...
      def remove_icon
        if @taxon.icon.destroy
          ...
          redirect_to aypex.edit_admin_taxonomy_taxon_url(@taxonomy.id, @taxon.id)
        else
      ...
      def successful_reposition_actions
        ...
        respond_to do |format|
          format.turbo_stream { render "aypex/admin/taxonomies/reposition" }
        end
      end
      ...
      def location_after_save
        aypex.edit_admin_taxonomy_taxon_path(@taxonomy.id, @taxon.id)
      end
