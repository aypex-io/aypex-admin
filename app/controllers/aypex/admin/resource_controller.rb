class Aypex::Admin::ResourceController < Aypex::Admin::BaseController
  ...
  def resource
    ...
    @resource = Aypex::Admin::Resource.new controller_path, controller_name, parent_model_name, object_name
  end
  ...
  def new_object_url(options = {})
    if parent_data.present?
      aypex.new_polymorphic_url([:admin, parent, model_class], options)
    else
      aypex.new_polymorphic_url([:admin, model_class], options)
    end
  end

  def edit_object_url(object, options = {})
    if parent_data.present?
      aypex.send "edit_admin_#{resource.model_name}_#{resource.object_name}_url",
        parent, object, options
    else
      aypex.send "edit_admin_#{resource.object_name}_url", object, options
    end
  end

  def clone_object_url(object, options = {})
    if parent_data.present?
      aypex.send "clone_admin_#{resource.model_name}_#{resource.object_name}_url",
        parent, object, options
    else
      aypex.send "clone_admin_#{resource.object_name}_url", object, options
    end
  end
  ...
  def object_url(object = nil, options = {})
    ...
    if parent_data.present?
      aypex.send "admin_#{resource.model_name}_#{resource.object_name}_url", parent, target, options
    else
      aypex.send "admin_#{resource.object_name}_url", target, options
    end
  end

  def collection_url(options = {})
    if parent_data.present?
      aypex.polymorphic_url([:admin, parent, model_class], options)
    else
      aypex.polymorphic_url([:admin, model_class], options)
    end
  end
