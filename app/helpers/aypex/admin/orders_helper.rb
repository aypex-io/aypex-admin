    ...
    module OrdersHelper
      ...
      def event_links(order, events)
        ...
        events.each do |event|
          ...
          label = Aypex.t(event, scope: "admin.order.events", default: Aypex.t(event))
          links << link_to_with_icon(label.capitalize, [event.to_sym, :admin, order], icon: event.to_s + ".svg", data: {turbo_method: :put, turbo_confirm: Aypex.t(:order_sure_want_to, event: label)}, class: "btn btn-secondary")
        end
        safe_join(links, "".html_safe)
      end

      def line_item_shipment_price(line_item, quantity)
        Aypex::Money.new(line_item.price * quantity, currency: line_item.currency)
      end

      def order_risk_state_badge(order)
        if order.considered_risky
          badge_style = "rgb-hsl-danger"
          badge_text = I18n.t("aypex.admin.risky")
        else
          ...
          badge_text = I18n.t("aypex.admin.safe")
        end
