    ...
    module BaseHelper
      ...
      def flash_alert(flash, turbo_frame_id = nil)
        ...
        if turbo_frame_id.nil?
          render "aypex/admin/shared/toast", message: flash.first[1], kind: flash.first[0]
        elsif flash[:turbo_frame_request_id] == turbo_frame_id.to_s
          render "aypex/admin/shared/toast", message: flash[:message], kind: flash[:kind]
        end
      end
      ...
      # Example:
      #
      #   aypex_humanize_dropdown_values('Aypex::CmsPage', { const: 'TYPES' })
      #   aypex_humanize_dropdown_values('Aypex::Menu', { const: 'MENU_TYPES', paramterize_values: true })
      #
      def aypex_humanize_dropdown_values(model_name, options = {})
        ...
        model_name.constantize.const_get(const).map do |type|
          ...
          formatted_option_values << [aypex_humanize_type(type), return_value]
        end

        formatted_option_values
      end

      def aypex_humanize_cms(cms_array)
        formatted_option_values = []

        cms_array.each do |section|
          formatted_option_values << [aypex_humanize_type(section.name), section]
        end

        formatted_option_values
      end

      def aypex_humanize_type(obj)
      ...
      def preference_fields(object, form)
        ...
        fields = object.preferences.keys.map do |key|
          if object.has_preference?(key)
            case key
            when :currency
              content_tag(:div, (form.select "#{key}", currency_options(object.preferences[key]), {}, {autocomplete: false, class: "form-select", data: {controller: "ts--select", form_state_target: "watch", input_disable_target: "disable"}}) +
                form.label("#{key}", I18n.t("aypex.admin.#{key}")),
              ...
              if object.preference_type(key).to_sym == :boolean
                content_tag(:div, preference_field_for(form, "#{key}", type: object.preference_type(key)) +
                  form.label("#{key}", I18n.t("aypex.admin.#{key}"), class: "form-check-label"),
                  class: "form-group form-check", id: [object.class.to_s.parameterize, "preference", key].join("-"))
              else
                content_tag(:div, preference_field_for(form, "#{key}", type: object.preference_type(key)) + form.label("#{key}", I18n.t("aypex.admin.#{key}")),
                  class: "form-group form-floating", id: [object.class.to_s.parameterize, "preference", key].join("-"))
              end
            end
          end
