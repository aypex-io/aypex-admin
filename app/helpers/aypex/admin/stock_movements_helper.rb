    ...
    module StockMovementsHelper
      def pretty_originator(stock_movement)
        if stock_movement.originator.respond_to?(:number)
          ...
          elsif stock_movement.originator.is_a?(Aypex::StockTransfer)
            link_to stock_movement.originator.number, aypex.admin_stock_transfer_url(stock_movement.originator)
          else
