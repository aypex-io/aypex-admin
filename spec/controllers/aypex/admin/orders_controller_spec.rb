...
class OrderSpecificAbility
  ...
  def initialize(_user)
    can [:admin, :manage], Aypex::Order, number: "R987654321"
  end
end

describe Aypex::Admin::OrdersController, type: :controller do
  ...
  context "with authorization" do
    ...
    let(:display_value) { Aypex::ShippingMethod::DISPLAY_ON_BACK_END }

    before do
      allow(controller).to receive(:try_aypex_current_user).and_return(admin_user)
    end
    ...
    context "search" do
      before do
        ...
        expect(Aypex::Order.count).to eq 1
      end

      def send_request
        get :index, params: {
          q: {
            line_items_variant_id_in: Aypex::Order.first.variant_ids
          }
        }
      end
      ...
      it "preloads users" do
        allow(Aypex::Order).to receive(:preload).with(:user).and_return(Aypex::Order.all)
        send_request
      end
    end
    ...
    describe "#open_adjustments" do
      ...
      context "when referer" do
        before do
          request.env["HTTP_REFERER"] = aypex.admin_url(host: "http://test.host")
        end

        it "redirects back" do
          put :open_adjustments, params: {id: order.number}
          expect(response).to redirect_to(aypex.admin_url(host: "http://test.host"))
        end
      end
  ...
  describe "#authorize_admin" do
    ...
    def with_ability(ability)
      Aypex::Ability.register_ability(ability)
      yield
    ensure
      Aypex::Ability.remove_ability(ability)
    end
    ...
    it "restricts returned order(s) on index when using OrderSpecificAbility" do
      ...
      expect(Aypex::Order.complete.count).to eq 4
      ...
      with_ability(OrderSpecificAbility) do
        ...
        expect(Aypex::Order.accessible_by(Aypex::Ability.new(user), :index).pluck(:number)).to eq [number]
      end
    end
  end
  ...
  context "wrong order number" do
    ...
    it "redirects to orders list" do
      ...
      expect(response).to redirect_to(aypex.admin_orders_path)
    end
  end
