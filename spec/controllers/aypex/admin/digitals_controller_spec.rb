...
RSpec.describe Aypex::Admin::DigitalsController do
    ...
    context "for a product that exists" do
      ...
      it "creates a digital associated with the variant and product" do
        expect do
          ...
          expect(response).to redirect_to(aypex.edit_admin_product_variant_path(product, variant))
        end.to change(Aypex::Digital, :count).by(1)
      end
    end
  end
    ...
    context "for a digital and product that exist" do
      it "deletes the associated digital" do
        expect do
          ...
          expect(response).to redirect_to(aypex.admin_product_digitals_path(product))
        end.to change(Aypex::Digital, :count).by(-1)
      end
    end
  end
