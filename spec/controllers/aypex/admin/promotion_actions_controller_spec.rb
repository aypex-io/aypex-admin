...
describe Aypex::Admin::PromotionActionsController, type: :controller do
  stub_authorization!

  let!(:promotion) { create(:promotion) }

  it "can create a promotion action of a valid type" do
    post :create, params: {promotion_id: promotion.id, action_type: "Aypex::Promotion::Actions::CreateItemAdjustments"}
  ...
  it "can not create a promotion action of an invalid type" do
    post :create, params: {promotion_id: promotion.id, action_type: "Aypex::InvalidType"}
    expect(response).to be_redirect
    expect(response).to redirect_to aypex.edit_admin_promotion_path(promotion)
