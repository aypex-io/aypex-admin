...
describe Aypex::Admin::ProductsController, type: :controller do
  stub_authorization!

  let(:store) { Aypex::Store.default }

  context "#index" do
    let(:ability_user) { stub_model(Aypex::LegacyUser, has_aypex_role?: true) }
  ...
  describe "#destroy" do
    let(:product) { mock_model(Aypex::Product) }
    ...
    context "will successfully destroy product" do
      before do
        allow(Aypex::Product).to receive(:friendly).and_return(products)
      ...
      describe "expects to receive" do
        ...
        it { expect(Aypex::Product).to receive(:friendly).and_return(products) }
    ...
    context "cannot destroy product from different store" do
      ...
      it { expect(send_request).to redirect_to(aypex.admin_products_path) }
  ...
  describe "#add_stock" do
    ...
    it "restricts stock location based on accessible attributes" do
      expect(Aypex::StockLocation).to receive(:accessible_by).and_return([])
      send_request
    end
