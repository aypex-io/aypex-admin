...
describe "Users", type: :feature do
  ...
  let!(:orders) { Aypex::Order.where(id: [order.id, order_2.id, order_eur.id, order_gbp.id]) }
  ...
  shared_examples_for "a user page" do
    context "lifetime stats" do
      ...
      context "when other store order exits" do
        ...
        it { expect(Aypex::Order.count).not_to eq(store.orders.count) }
  ...
  before do
    store.address_show_company_address_field = true

    create(:country)
    stub_const("Aypex::User", create(:user, email: "example@example.com").class)
  ...
  context "editing users", js: true do
    ...
    it "can edit user roles" do
      ...
      within("#universalModal") do
        expect(page).to have_checked_field("user_aypex_role_admin")
      end
    end
  ...
  context "order history with sorting" do
    before do
      orders
      visit aypex.orders_admin_user_path(user_a)
    end
